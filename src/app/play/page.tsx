...
function PlayPageClient() {
  ...
  const preferBestSource = async (sources: SearchResult[], signal?: AbortSignal): Promise<SearchResult> => {
    ...
    type TestResult = 
      | { status: 'ok'; source: SearchResult; testResult: { quality: string; loadSpeed: string; pingTime: number } }
    ...
    const testSource = async (source: SearchResult): Promise<TestResult> => {
      ...
      try {
        if (!source.episodes || source.episodes.length === 0) {
          if (debugEnabledRef.current) console.warn(`播放源 ${source.source_name} 没有可用的播放地址`);
          return { status: 'error', source };
        }
        const episodeUrl = source.episodes.length > 1 ? source.episodes[1] : source.episodes[0];
        
        const testResult = await getVideoResolutionFromM3u8(episodeUrl, { signal });
    ...
    const runWithConcurrencyLimit = async (inputTasks: (() => Promise<TestResult>)[], limit: number): Promise<TestResult[]> => {
    ...
    if (!signal?.aborted) {
      ...
      allResults.forEach((result) => {
        ...
        if (result.status === 'ok') {
          ...
          const sourceKey = `${result.source.source}-${result.source.id}`;
          newVideoInfoMap.set(sourceKey, { quality: 'Error', loadSpeed: '0 KB/s', pingTime: 0, hasError: true });
        }
      });
    ...
    const successfulResults = allResults
      .filter((r): r is Extract<TestResult, { status: 'ok' }> => r?.status === 'ok')
      .map(r => ({ source: r.source, testResult: r.testResult }));

    if (successfulResults.length === 0) {
      if (debugEnabledRef.current) console.warn('所有播放源测速都失败，使用第一个播放源');
      return sources[0];
    }
  ...
  const handleNextEpisode = () => {
    if (detail && detail.episodes && currentEpisodeIndex < detail.episodes.length - 1) {
      ...
      <div className='flex flex-col gap-3 py-0 md:py-4 px-0 md:px-5 lg:px-12 2xl:px-20'>
        ...
        <div className='space-y-2'>
          ...
          <div className={`grid gap-4 lg:h-[500px] xl:h-[650px] 2xl:h-[750px] transition-all duration-300 ease-in-out ${isEpisodeSelectorCollapsed ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-4'}`}>
            ...
            <div className={`h-[300px] lg:h-full md:overflow-hidden transition-all duration-300 ease-in-out ${isEpisodeSelectorCollapsed ? 'md:col-span-1 lg:hidden lg:opacity-0 lg:scale-95' : 'md:col-span-1 lg:opacity-100 lg:scale-100'} ${isVideoLoading ? 'opacity-80' : ''}`}>
              <EpisodeSelector
