...
function HomeClient() {
  ...
  useEffect(() => {
    ...
    fetchData(
      ...
      (data: DoubanResponse) => {
        if (data.code === 200)
          setCategories((prev) => ({ ...prev, movies: data.list }));
      },
    );
    ...
    fetchData(
      ...
      (data: DoubanResponse) => {
        if (data.code === 200)
          setCategories((prev) => ({ ...prev, tv: data.list }));
      },
    );
    ...
    fetchData(
      ...
      (data: DoubanResponse) => {
        if (data.code === 200)
          setCategories((prev) => ({ ...prev, variety: data.list }));
      },
    );
  ...
  const updateFavoriteItems = useCallback(
    async (allFavorites: FavoriteRecord) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToDataUpdates('favoritesUpdated', (data) => {
      if (isMountedRef.current)
        updateFavoriteItems(data as unknown as FavoriteRecord);
    });
    return unsubscribe;
  ...
  const todayAnimes = useMemo(() => {
    return (
      categories.bangumi.find((item) => item.weekday.en === currentWeekday)
        ?.items || []
    );
          ...
          {activeTab === 'favorites' ? (
            ...
            <>
              ...
              <section className='mb-8'>
                ...
                <ScrollableRow>
                  {loadingStates.variety ? (
                    <SectionSkeleton />
                  ) : (
                    categories.variety.map((show, index) => (
                      <div
                        key={index}
                        className='min-w-[96px] w-24 sm:min-w-[180px] sm:w-44'
                      >
